            ...
            <div class="border-2 border-dashed border-gray-300 rounded-lg p-6">
                ...
                @if($document)
                    <div class="mt-4 text-sm text-gray-600">
                        Selected file: {{ $document ? $document->getClientOriginalName() : 'No file selected' }}
                    </div>
                @endif
            </div>
        ...
        @foreach($entities as $entity)
            <div class="mb-8 border rounded-lg p-4">
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Affair</th>
                                ...
                                <tr>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Sub Organization</th>
                ...
                @foreach($entity->programs as $program)
                    <div class="ml-4 mb-4">
                        <h4 class="text-lg font-semibold mb-2">Program: {{ $program->name }}</h4>
                        
                        @foreach($program->activities as $activity)
                            <div class="ml-4 mb-4">
                                <h5 class="text-md font-semibold mb-2">Activity: {{ $activity->name }}</h5>
                                
                                @foreach($activity->subActivities as $subActivity)
                                    <div class="ml-4 mb-4">
                                        <h6 class="font-semibold mb-2">Sub Activity: {{ $subActivity->name }}</h6>
                                                ...
                                                <tbody class="bg-white divide-y divide-gray-200">
                                                    @foreach($subActivity->items as $item)
                                                        <tr>
                                                            ...
                                                            <td class="px-6 py-4 whitespace-nowrap">{{ number_format($item->tax, 2) }}</td>
